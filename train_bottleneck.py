...
def _main():
    annotation_path = '/home/drive/data/underwater/data/train_image_annotation.txt'
    log_dir = 'logs/bottlenect/'
    classes_path = 'model_data/underwater_classes.txt'
    anchors_path = 'model_data/yolov3_underwater_anchors.txt'
    ...
    model, bottleneck_model, last_layer_model = create_model(input_shape, anchors, num_classes,
            freeze_body=2, weights_path='model_data/yolo-water.h5') # make sure you know what you freeze
    ...
    npz_count=int(len(lines)/500)#Maxcount is 1700 Beacuse MEM

    # Train with frozen layers first, to get a stable loss.
    # Adjust num epochs to your dataset. This step is enough to obtain a not bad model.
    if True:
        for i in range(0,npz_count+1):
            filename="bottlenecks_"+str(i)+".npz"
            # perform bottleneck training
            if not os.path.isfile(filename):
                print("calculating bottlenecks")
                batch_size=8
                end_num=(i+1)*500 if len(lines)>=(i+1)*500 else len(lines)
                data_wapper=data_generator_wrapper(lines[i*500:end_num], batch_size, input_shape, anchors, num_classes, random=False, verbose=True)
                bottlenecks=bottleneck_model.predict_generator(data_wapper,
                steps=((end_num-i*500)//batch_size)+1, max_queue_size=1)
                np.savez(filename, bot0=bottlenecks[0], bot1=bottlenecks[1], bot2=bottlenecks[2])
                print("Save Success")

        # load bottleneck features from file
        dict_bot=np.load("bottlenecks_0.npz")
